...
This repo is an **npm-workspaces monorepo** (`setu-web` + `setu-convex`
...
Vercel). `setu-convex/convex/_generated` is committed on purpose.
...
3. **Demo mode**: for the SIH demo URL, set `DEV_AUTH=1` on the deployment
   ...
   public launch, replace the shim with ConvexAuth (`setu-convex/CONVEX_AUTH.md`)
   and remove `DEV_AUTH`.
4. **Vercel**: push this monorepo to GitHub → Import Project → set
   **Root Directory = `setu-web`** → add env var
...
5. **AI worker (optional)**: deploy `setu-ai-service` (Dockerfile provided) and set
